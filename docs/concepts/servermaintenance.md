...
5. The `ServerMaintenanceReconciler` creates a ServerBootConfiguration out of the ServerMaintenance's
   `ServerBootConfigurationTemplate` and applies it to the Server. The power state of the Server can be set by providing the
...
If `policy` is OwnerApproval and no `ok-to-maintenance` label exists on the ServerClaim, this ServerMaintenance
